...
def imprime_variaveis(net):
    tds_dist = 1
    for var in net.variables:
        print(var, ": ", var.domain, "Parents:",var.parents)
        tds_dist *= len(var.domain)
    print("Todas as linhas possíveis da distribuição conjunta:",tds_dist)
...
with open('diabetes.pkl', 'rb') as entrada:
    diabetes_net = pickle.load(entrada)
    globalize(diabetes_net.lookup)
    print(diabetes_net.variables)
    imprime_variaveis(diabetes_net)
    print(set(all_rows(diabetes_net)))
    ea = enumeration_ask(Diabetes, {Body_Mass_Index:'overweight',Pregnancies:'few',Insulin_Test:'normal'}, diabetes_net)
    print(ea)
